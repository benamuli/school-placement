...
if(isset($_POST['submit'])){
        ...
        echo "selection submitted succesfully ";
        $sql = "INSERT INTO schools_selected (index_no,marks,email,national,extra_county,county) values ('$index_no','$marks','$email','$selected1','$selected2','$selected3')";
        $result = mysqli_query($con, $sql);
            echo "<script>alert('selection submited successfully')</script>";
        }
        else{
...
<head>
    ...
    <title>HISEB school selection portal</title>
</head>
...
<body>
    ...
    <form method="POST">
        ...
        <?php
        } else if ($_SESSION['marks'] >= 350 && $_SESSION['marks'] < 400) { ?>
            <div style=" margin: auto 100px;"><label for="category">EXTRA COUNTY</label></div>
            <select style=" margin: auto 100px;" class="form-select" size="5" aria-label="size 8 select example" name="extra_county">
                <?php
                $sql = "SELECT * FROM schools  WHERE level='Extra county '";
